...
class LanguageDetails: NSObject {
    ...
    override init() {
        ...
        bundle = Bundle(path: Bundle.main.path(forResource: (language == "ru") ? language : (language == "en") ? language : "el", ofType: "lproj")!)
...
//        }
    ...
    {
        let changeTo: String = langString
...
//        IQKeyboardManager.shared.toolbarDoneBarButtonItemText = languageHelper.LocalString(key: "Done_Title")
    ...
    func getLocale() -> Locale {
        if language == "el" {
            return Locale.init(identifier: "el_CY")
        }else if language == "ru" {
...
//        let bundl:Bundle! = Bundle(path: Bundle.main.path(forResource: lan == "de" ? lan : "en", ofType: "lproj")!)
